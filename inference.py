...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run inference and convert audio to MIDI"
    )

    parser.add_argument(
        "--ckpt", type=str, required=True, help="Path to transcriber checkpoint"
    )
    parser.add_argument(
        "--audio-files-dir",
        type=str,
        required=True,
        help="Directory containing FLAC files for inference",
    )
    parser.add_argument(
        "--out-dir",
        type=str,
        required=True,
        help="Output directory for saving MIDI files",
    )

    parser.add_argument(
        "--save-onsets-and-frames",
        action="store_true",
        help="Save raw onset/frame predictions as .npy files",
    )
...
def inference_single_flac(
    transcriber, flac_path, inst_mapping, out_dir, save_onsets_and_frames=False
):
    audio = load_audio(flac_path)
    audio_inp = audio.float() / 32768.0
    ...
    if audio_inp_len > MAX_TIME:
        ...
        for i_s in range(n_segments):
            ...
            curr_onset_pred, curr_offset_pred, _, curr_frame_pred, *_ = transcriber(
                curr_mel
            )
    ...
    save_path = os.path.join(
        out_dir, os.path.basename(flac_path).replace(".flac", ".mid")
    )
    ...
    if save_onsets_and_frames:
        onset_save_path = os.path.join(
            out_dir, os.path.basename(flac_path).replace(".flac", "_onset_pred.npy")
        )
        frame_save_path = os.path.join(
            out_dir, os.path.basename(flac_path).replace(".flac", "_frame_pred.npy")
        )
...
def inference_multiple_flacs(
    transcriber_ckpt, flac_dir, config, save_onsets_and_frames=False
):
    ...
    flac_path_list = [
        os.path.join(flac_dir, f) for f in os.listdir(flac_dir) if f.endswith(".flac")
    ]
