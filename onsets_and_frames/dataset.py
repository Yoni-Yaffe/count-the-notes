...
class EMDATASET(Dataset):
    def __init__(self,
        ...
        self.eval_file_list = []
        self.file_list = self.files(self.groups, pitch_shift=pitch_shift, keep_eval_files=keep_eval_files, n_eval=n_eval, evaluation_list=evaluation_list, pitch_shift_limit=pitch_shift_limit)
    ...
    def files(self, groups, pitch_shift=True, keep_eval_files=False, n_eval=1, evaluation_list=None, pitch_shift_limit=5):
    ...
    def __getitem__(self, index):
        ...
        audio = data['audio'][begin:end].float().div_(32768.0)  # torch.ShortTensor → float
        ...
        if label.shape[0] < n_steps:
            ...
            label = torch.cat([label,
                            torch.zeros((pad_amt, *label.shape[1:]),
                                        dtype=label.dtype)], dim=0)
        ...
        if self.use_onset_mask:
            if 'onset_mask' in data:
                result['onset_mask'] = data['onset_mask'][step_begin:step_end, ...].float()
            else:
                result['onset_mask'] = torch.ones_like(result['onset']).float()
            if 'frame_mask' in data:
                result['frame_mask'] = data['frame_mask'][step_begin:step_end, ...].float()
            else:
        ...
        for flac, tsv in tqdm(files, desc='loading pts'):
            ...
            if os.path.isfile(self.labels_path + os.sep +
                ...
                if len(unaligned_label) < self.sequence_length // constants.HOP_LENGTH:
                    diff = self.sequence_length // constants.HOP_LENGTH - len(unaligned_label)
                    pad = torch.zeros((diff, unaligned_label.shape[1]), dtype=unaligned_label.dtype)
    ...
    def update_pts_counting(self, transcriber, counting_window_length, POS=1.1, NEG=-0.001, FRAME_POS=0.5,
                   to_save=None, first=False, update=True, BEST_DIST=False, peak_size=3, BEST_DIST_VEC=False, counting_window_hop=0):
        print('Updating pts...')
        print(f"First {first}")
        ...
        for input_files in tqdm(onlt_pitch_0_files, desc='updating pts'):
            ...
            if audio_inp_len > MAX_TIME:
                ...
                print("number of needed segments is ", int(np.ceil(audio_inp_len / MAX_TIME)))
                seg_len = MAX_TIME
                onsets_preds = []
                offset_preds = []
                frame_preds = []
            ...
            if peak_size > 0:
                peaks = get_peaks(onset_pred, PEAK_SIZE)  # we only want local peaks, in a 7-frame neighborhood, 3 to each side.
                onset_pred[~peaks] = 0
            ...
            bon_dist_vec = np.abs(pred_bag_of_notes_with_inst - gt_bag_of_notes_with_inst)
            ...
            print(f"unaligned onsets shape {unaligned_onsets.shape} counting window length {counting_window_length} counting window hop {counting_window_hop}")
            assert counting_window_hop <= counting_window_length
            if counting_window_hop == 0:
                counting_window_hop = counting_window_length
            
            num_chunks = 1 if counting_window_length == 0 else int(np.ceil(len(unaligned_onsets) / counting_window_hop))
            ...
            for chunk_idx in range(num_chunks):
                ...
                if counting_window_length == 0:
                    ...
                    end_idx = min(start_idx + counting_window_length, len(unaligned_onsets))
                chunk_onsets = unaligned_onsets[start_idx:end_idx]
                chunk_onsets_count = (data['unaligned_label'][start_idx:end_idx,:] == 3).sum(dim=0).numpy()
                
                for f, f_count in enumerate(chunk_onsets_count):
                    if f_count == 0:
                        continue
                    f_most_likely = np.sort(onset_pred_np[start_idx:end_idx, f].argsort()[::-1][:f_count])
            ...
            print(f'Counting alignment for file "{flac}" took {counting_duration:.2f} seconds.')
            ...
            if update:
                if BEST_DIST_VEC:
                    ...
                    if prev_bon_dist_vec is None:
                        raise ValueError("BEST_DIST_VEC is True but no previous BON_VEC found")
            ...
            if bon_dist < prev_bon_dist:
                print(f'Bag of notes distance improved from {prev_bon_dist} to {bon_dist}')
                data['BON'] = bon_dist
        ...
        print(f'Total counting alignment time for all files: {total_counting_time:.2f} seconds.')
